...
export default function SellerSettingsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/seller/settings', { headers: { 'Authorization': `Bearer ${token}` } })
      .then(r => r.json()).then(d => { if (d.settings) setSettings(d.settings); }).catch(console.error).finally(() => setLoading(false));
  }, [router]);
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch('/api/seller/settings', { method:'PUT', headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`}, body:JSON.stringify(settings) });
  ...
  if (!mounted || loading) return <div className="min-h-screen flex items-center justify-center"><p className="text-gray-600">Loading settings...</p></div>;
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-8 space-y-6">
          ...
          <button type="submit" disabled={saving} className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 font-semibold">{saving?'Saving...':'Save Settings'}</button>
        </form>
      </div>
    </div>
  );
