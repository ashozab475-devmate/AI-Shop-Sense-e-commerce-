#!/usr/bin/env python3
"""
train_2000_products.py
──────────────────────
Train CLIP + Classifier with 2000 products from the dataset

This script will:
1. Load 2000 products from abo_dataset_6000.csv
...
CSV_PATH    = DATA_DIR / 'abo_dataset_2000_actual.csv'
...
MAX_PRODUCTS    = 2000  # ← Use all 2000 products from the CSV
...
def load_image(path_str: str):
    """Try multiple path resolutions to load an image."""
...
def main():
    ...
    logger.info(f"Training with {MAX_PRODUCTS} products")
    ...
    # Randomly sample MAX_PRODUCTS rows
    if len(all_rows) > MAX_PRODUCTS:
        random.seed(42)  # For reproducibility
        rows = random.sample(all_rows, MAX_PRODUCTS)
        logger.info(f"Randomly sampled {MAX_PRODUCTS} products")
    else:
        rows = all_rows
        logger.info(f"Using all {len(rows)} products")
    ...
    logger.info("\n✓ Training complete!")
    logger.info(f"  Total products:  {len(embeddings)}")
    ...
    logger.info("\nRestart the search service to use the new model.")
